...
//		buf.writeInt(this.itemDamage);
	...
	public static void handle(final SpellUpdateS2C message, Supplier<NetworkEvent.Context> ctxSupplier) {
		...
		if (sideReceived != LogicalSide.CLIENT) {
			MagicThings.LOGGER.warn("SpellUpdateS2C received on wrong side -> {}", ctx.getDirection().getReceptionSide());
			return;
		}
		if (!message.isValid()) {
			MagicThings.LOGGER.warn("SpellUpdateS2C was invalid -> {}", message.toString());
			return;
		}
		...
		if (clientLevel.isEmpty()) {
			MagicThings.LOGGER.warn("SpellUpdateS2C context could not provide a ClientLevel.");
			return;
		}
	...
	private static void processMessage(Level level, SpellUpdateS2C message) {
		MagicThings.LOGGER.debug("received spell message -> {}", message);
		try {
			Player player = level.getPlayerByUUID(message.getPlayerId());
			if (player != null) {
				MagicThings.LOGGER.debug("valid player -> {}", message.getPlayerId());
				// check hands first
				if (message.getHand() != null) {
					MagicThings.LOGGER.debug("hand handler - valid hand -> {}", message.getHand());
					...
					if (heldItemStack != null) {
						MagicThings.LOGGER.debug("holding item -> {}", heldItemStack.getItem().getRegistryName());
						updateJewelry(player, heldItemStack, message);
					}
				}
				else if (CURIOS_ID.equals(message.getSlotProviderId())) {
					MagicThings.LOGGER.debug("curios handler - updating slot spell...");
					...
					curiosHandler.ifPresent(itemHandler -> {
						...
						stacksOptional.ifPresent(stacksHandler -> {
							MagicThings.LOGGER.debug("# of slots in curios handler -> {}", stacksHandler.getStacks().getSlots());
					...
					MagicThings.LOGGER.debug("hotbar handler");
					// get the item from the hotbar
					ItemStack hotbarStack = player.getInventory().getItem(Integer.valueOf(message.getSlot()));
					if (hotbarStack != null) {
						MagicThings.LOGGER.debug("hotbar item -> {}", hotbarStack.getItem().getRegistryName());
			...
			MagicThings.LOGGER.error("Unexpected error ->", e);
		}
	}
	...
	private static void updateJewelry(Player player, ItemStack itemStack, SpellUpdateS2C message) {
		itemStack.getCapability(MagicThingsCapabilities.JEWELRY_CAPABILITY).ifPresent(handler -> {
			...
			if (!entityList.isEmpty() && entityList.size() > message.getIndex()) {
				...
				MagicThings.LOGGER.debug("looking for spell -> {} at index -> {}", entity, message.getIndex());
